...
# Determine available Docker Compose command (prefer plugin, fall back to legacy binary)
...
$(error "Docker Compose is required. Install 'docker compose' (Docker CLI plugin) or 'docker-compose'.")
endif
...
build: check_docker
	...
	@$(COMPOSE_BUILD_ENV) $(DOCKER_COMPOSE_CMD) build
...
migrate-or-init: check_docker
	...
	set -e; \
	if ! $(DOCKER_COMPOSE_CMD) ps -q postgres | grep -q .; then \
	echo "ERROR: PostgreSQL container is not running. Please run 'make up' first."; \
	exit 1; \
	fi; \
	echo "---> Waiting for database to be ready..."; \
	count=0; \
	while ! $(DOCKER_COMPOSE_CMD) exec -T postgres pg_isready -U postgres -d vexa -q; do \
	if [ $$count -ge 12 ]; then \
	...
	if $(DOCKER_COMPOSE_CMD) exec -T postgres psql -U postgres -d vexa -t -c "SELECT 1 FROM information_schema.tables WHERE table_name = 'alembic_version';" | grep -q 1; then \
	echo "STATE: Alembic-managed database detected."; \
	echo "ACTION: Running standard migrations to catch up to 'head'..."; \
	$(MAKE) migrate; \
	elif $(DOCKER_COMPOSE_CMD) exec -T postgres psql -U postgres -d vexa -t -c "SELECT 1 FROM information_schema.tables WHERE table_name = 'meetings';" | grep -q 1; then \
	...
	$(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini stamp base; \
	$(MAKE) migrate; \
	else \
	...
	$(DOCKER_COMPOSE_CMD) exec -T transcription-collector python -c "import asyncio; from shared_models.database import init_db; asyncio.run(init_db())"; \
	$(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini stamp dc59a1c03d1f; \
	fi; \
...
migrate: check_docker
	...
	@current_version=$$($(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini current 2>/dev/null | grep -E '^[a-f0-9]{12}' | head -1 || echo ""); \
	if [ "$$current_version" = "dc59a1c03d1f" ]; then \
	if $(DOCKER_COMPOSE_CMD) exec -T postgres psql -U postgres -d vexa -t -c "SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'data';" | grep -q 1; then \
	echo "---> Preflight: detected existing column users.data. Stamping 5befe308fa8b..."; \
	$(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini stamp 5befe308fa8b; \
	fi; \
	fi
	@echo "---> Running alembic upgrade head..."
	@$(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini upgrade head
...
makemigrations: check_docker
	...
	@$(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini revision --autogenerate -m "$(M)"
...
init-db: check_docker
	...
	$(DOCKER_COMPOSE_CMD) run --rm transcription-collector python -c "import asyncio; from shared_models.database import init_db; asyncio.run(init_db())"
	$(DOCKER_COMPOSE_CMD) run --rm transcription-collector alembic -c /app/alembic.ini stamp head
...
stamp-db: check_docker
	...
	@$(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini stamp head
...
migration-status: check_docker
	...
	@$(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini current
	@echo "---> Migration history:"
	@$(DOCKER_COMPOSE_CMD) exec -T transcription-collector alembic -c /app/alembic.ini history --verbose
